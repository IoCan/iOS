...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        self.title = @"服务窗";
        _topItems = @[@"消息通知", @"备胎消息", @"优惠消息"];
        _tabCount = _topItems.count;
    ...
    [manager POST:[BaseUrlString stringByAppendingString:@"sysmessagequery.do"] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        @try {
            ...
            if ([@"00" isEqualToString:result]) {
                NSMutableArray *data = [responseObject objectForKey:@"resultlist"];
...
-(void) initSlideView{
    CGFloat width = ScreenWidth / default_count;
    if(self.tabCount <= default_count){
        width = ScreenWidth / self.tabCount;
    }
...
-(void) initScrollView{
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, default_topview_height, ScreenWidth, ScreenHeight - default_topview_height-49-64)];
    _scrollView.contentSize = CGSizeMake(ScreenWidth * _tabCount, ScreenHeight - default_topview_height-64-49);
...
-(void) initTopTabs{
    ...
    _topMainView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, default_topview_height)];
    _topScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, default_topview_height+1)];
    ...
    if (_tabCount >= default_count) {
        ...
        _topScrollView.contentSize = CGSizeMake(ScreenWidth, default_topview_height);
    }
    ...
    @try {
        NSArray *tmpdata = [_dataSource[0] filteredArrayUsingPredicate:predicate];
        NSArray *tmpbtdata = [_dataSource[1] filteredArrayUsingPredicate:predicate];
        NSArray *tmpyhdata = [_dataSource[2] filteredArrayUsingPredicate:predicate];
        
        
        label1.hidden = (tmpdata.count == 0);
        label1.text = [NSString stringWithFormat:@"%ld",(unsigned long)tmpdata.count];
        
        label2.hidden = (tmpbtdata.count == 0);
        label2.text = [NSString stringWithFormat:@"%ld",(unsigned long)tmpbtdata.count];
        
        label3.hidden = (tmpyhdata.count == 0);
        label3.text = [NSString stringWithFormat:@"%ld",(unsigned long)tmpyhdata.count];
    }
    @catch (NSException *exception) {
...
-(void) tabButton: (id) sender{
    ...
    [_scrollView setContentOffset:CGPointMake((button.tag-1) * ScreenWidth, 0) animated:YES];
}
...
-(void) initDownTables{
    ...
    for (int i = 0; i < 3; i ++) {
        UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(i * ScreenWidth, 0, ScreenWidth ,ScreenHeight - default_topview_height - 49 - 64)];
        tableView.delegate = self;
        tableView.dataSource = self;
        tableView.tag = i;
        if (i<2) {
            tableView.header = [MJRefreshNormalHeader headerWithRefreshingTarget:self refreshingAction:@selector(updatedata)];
        }
        ...
        label.tag = 10+i;
        label.textAlignment = NSTextAlignmentCenter;
        label.frame = CGRectMake(ScreenWidth/2-100+i*ScreenWidth, tableView.height/2-20, 200, 40);
        label.hidden = YES;
    ...
    //文字提示无数据
    NSMutableArray *a1 = _dataSource[0];
    NSMutableArray *a2 = _dataSource[1];
    NSMutableArray *a3 = _dataSource[2];
    [_scrollView viewWithTag:10].hidden = (a1.count > 0);
    [_scrollView viewWithTag:11].hidden = (a2.count > 0);
    [_scrollView viewWithTag:12].hidden = (a3.count > 0);
    ...
    [manager POST:[BaseUrlString stringByAppendingString:@"sysmessagequery.do"] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        @try {
            ...
            if ([@"00" isEqualToString:result]) {
                ...
                label.text = [NSString stringWithFormat:@"%ld",(unsigned long)tmpdata.count];
...
-(void) updateTableWithPageNumber: (NSUInteger) pageNumber{
    ...
    CGRect tableNewFrame = CGRectMake(pageNumber * ScreenWidth, 0, ScreenWidth, ScreenHeight - default_topview_height-64-49);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if ([scrollView isEqual:_scrollView]) {
        ...
        _currentPage = _scrollView.contentOffset.x/ScreenWidth;
        _currentPage = _scrollView.contentOffset.x/ScreenWidth;
